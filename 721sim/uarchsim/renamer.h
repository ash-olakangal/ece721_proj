...
public:
    ...
    PhaseCircularFIFO(size_t size) : buffer(size), capacity(size) {}
    ...
    bool push(T val) {
        ...
        if (tail == capacity) {
            tail = 0;
            tail_phase = !tail_phase; // Flip phase on wrap-around
        }
        return true;
    }

    // TODO: Check for is_empty() before calling front and pop!!!
    T& front() {
        return buffer[head];
    }
...
private:
	...
	uint64_t NUM_LOGICAL_REG;
	uint64_t NUM_PHYSICAL_REG;
	uint64_t NUM_BRANCH_CP;
	uint64_t AL_SIZE;

    struct AL_entry{
        bool dest_flag;
        uint64_t logical_dest;
        uint64_t physical_dest;
        bool completed;
        bool exception;
        bool load_violation;
        bool branch_mis;
        ...
        AL_entry() : dest_flag(false), logical_dest(0), physical_dest(0), completed(false), exception(false), load_violation(false), branch_mis(false), value_mis(false), load_flag(false), store_flag(false), branch_flag(false), amo_flag(false), csr_flag(false), instr_pc(0){}
    };

    struct BC_entry{
        std::vector <uint64_t> CP_RMT;
        uint64_t FL_head_index;
        bool head_phase;
        bool tail_phase;
        uint64_t CP_GBM;
	bool free; // NOTE: initially this is true all entries are free in branch CP

        BC_entry(uint64_t NUM_LOGICAL_REG) : CP_RMT(NUM_LOGICAL_REG, 0), FL_head_index(0), head_phase(false), tail_phase (false), CP_GBM(0), free(true){}
    };
	...
	/////////////////////////////////////////////////////////////////////
    PhaseCircularFIFO<uint64_t> FreeList;
	...
	// * Structure includes head, tail, and their phase bits.
	/////////////////////////////////////////////////////////////////////
    PhaseCircularFIFO<AL_entry> ActiveList;
	...
	/////////////////////////////////////////////////////////////////////
    std::vector<bool> PRF_ready; // NOTE : Should be true intially as all entries are free in PRF
	...
	// 3. checkpointed GBM
	/////////////////////////////////////////////////////////////////////
    std::vector<BC_entry> Branch_Checkpoint;

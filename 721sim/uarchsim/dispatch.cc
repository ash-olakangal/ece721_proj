...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
	...
	PAY.buf[index].AL_index = REN->dispatch_inst(PAY.buf[index].C_valid, PAY.buf[index].C_log_reg, PAY.buf[index].C_phys_reg, load_flag, store_flag,  branch_flag, amo_flag, csr_flag, PAY.buf[index].pc);
      ...
      // FIX_ME #8 BEGIN
        A_ready = (!PAY.buf[index].A_valid) ? true : REN->is_ready(PAY.buf[index].A_phys_reg);
	B_ready = (!PAY.buf[index].B_valid) ? true : REN->is_ready(PAY.buf[index].B_phys_reg);
	D_ready = (!PAY.buf[index].D_valid) ? true : REN->is_ready(PAY.buf[index].D_phys_reg);
      // FIX_ME #8 END
      ...
      //aolakan
        if (PAY.buf[index].C_valid) {
	   if (PAY.buf[index].vp_confident && PAY.buf[index].vp_pred_avail) {
	      REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].vp_value.dw);
	      REN->set_ready(PAY.buf[index].C_phys_reg);
	      PAY.buf[index].vp_used = true;
	   }
	   else {
	      REN->clear_ready(PAY.buf[index].C_phys_reg);
